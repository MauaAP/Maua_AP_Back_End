  ...
  async createEvent(event: Event): Promise<Event> {
    try {
      ...
      const createdEvent = new Event({
        ...
        date: createdEventFromPrisma.date ? createdEventFromPrisma.date.getTime() : new Date().getTime(),
    ...
    try {
      ...
      const events = eventsFromPrisma.map((event) => {
        return new Event({
          eventId: event.id,
          eventName: event.eventName,
          date: event.date ? event.date.getTime() : new Date().getTime(),
          host: event.host,
          manager: event.manager || [],
          hostEmail: event.hostEmail || [],
          hostPhone: event.hostPhone || [],
  ...
  async getEventById(eventId: string): Promise<Event> {
    try {
      ...
      const event = new Event({
        ...
        date: eventFromPrisma.date ? eventFromPrisma.date.getTime() : new Date().getTime(),
